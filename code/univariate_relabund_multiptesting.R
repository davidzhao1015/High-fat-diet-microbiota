...
genus_diet_rel_abund <- inner_join(genus_meta2, genus_pool, by="genus") %>%
        ...
        mutate(genus = factor(genus),
               genus = fct_reorder(genus, median, .desc = F)) 
...
boxplot_genus_stat +
        geom_segment(data = genus_significance, 
                     aes(x= 2, xend =2, y=y, yend=yend),
                     inherit.aes = F) +
        geom_text(data = genus_significance, 
                  aes(x= 1.5*2, y=y+0.15),
                  label="*", 
                  inherit.aes = FALSE)


ggsave("C:/Users/17803/Documents/RStuodio-link-GitHub/Wu_2011_MLRepo/High-fat-diet-microbiota/documentation/boxplot_genus_star.tiff",
       width = 4, height=5)
